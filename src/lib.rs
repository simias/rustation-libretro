...
use libc::{c_char, c_int, c_uint, uint8_t, int16_t, uint16_t, uint32_t};

use retrogl::RetroGl;
use renderer::{CommandVertex, SemiTransparencyMode};
...
pub extern "C" fn rsx_push_triangle(p0x: int16_t,
    ...
    let (semi_transparent, blend_mode) =
        match blend_mode {
            -1 => (false, SemiTransparencyMode::Add),
            0 => (true, SemiTransparencyMode::Average),
            1 => (true, SemiTransparencyMode::Add),
            2 => (true, SemiTransparencyMode::SubstractSource),
            3 => (true, SemiTransparencyMode::AddQuarterSource),
            _ => unreachable!(),
        };

    let v = [
        CommandVertex {
            position: [p0x as i16, p0y as i16, 0],
        ...
        CommandVertex {
            position: [p1x as i16, p1y as i16, 0],
        ...
        CommandVertex {
            position: [p2x as i16, p2y as i16, 0],
    ...
    renderer().gl_renderer().push_triangle(v, blend_mode);
}
